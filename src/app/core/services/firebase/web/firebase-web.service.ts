...
export class FirebaseWebService extends FirebaseService implements OnDestroy{
  ...
  async init(){
    const firebaseConfig = {
      apiKey: "AIzaSyCCzIGuhfo0JBrOpvIMHEmdF_e2PR-s36M",
      authDomain: "controlnotas-ncm.firebaseapp.com",
      projectId: "controlnotas-ncm",
      storageBucket: "controlnotas-ncm.appspot.com",
      messagingSenderId: "875002384688",
      appId: "1:875002384688:web:5dff826f1087e754ebb8f5"
    };
  ...
  ngOnDestroy(): void {
    deleteApp(this.app).then((value: any)=>{
      this.analytics = null;
      this.active = false;
    }).catch((reason: any)=>{
  ...
  public getDocuments(collectionName:string, orderByField:string="", orderMode:OrderByDirection="asc"):Promise<FirebaseDocument[]>{
    return new Promise(async (resolve, reject)=>{
      if(orderByField!=""){
          const q = query(collection(this.db, collectionName), orderBy("orderBy", orderMode));
  ...
  public async createUserWithEmailAndPassword(email:string, password:string):Promise<UserCredential>{
    try {
      var create:any = createUserWithEmailAndPassword(this.auth, email, password);
    } catch (error: any) {
      switch (error.code) {
